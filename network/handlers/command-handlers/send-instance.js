...
module.exports.handle = (player, playerName, instanceName) => {
    ...
    if (!targetInstance) {
        return player.send.message(`instance @yel@${instanceName} @whi@ does not exist`)
    }

    targetPlayer.instance.removePlayer(targetPlayer)
    targetInstance.addPlayer(targetPlayer)

    player.send.message(`@yel@${playerName} @whi@was moved to the @yel@${instanceName} @whi@instance`)
    targetPlayer.send.message(`you have been warped inside @yel@${instanceName}`)
}
